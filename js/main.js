...
jQuery(document).ready(function ($) {
  ...
  const ANNIE = {
    ...
    loadAnimation:
      '<div class = "transition"><div class = "three-bounce1"> </div> <div class = "three-bounce2"> </div> <div class = "three-bounce3"> </div> </div> ',
  };
  ...
  const Annie_Preloader = function () {
    ...
    const PRELOADER = {
      ...
      getCookie: function (cName) {
        ...
        for (let i = 0; i < aCookie.length; i++) {
          ...
          if (cName == aCrumb[0]) return unescape(aCrumb[1]);
        }
        return 0;
      },
    ...
    function singlePageScroll() {
      if (browserRefreshStatus) {
        currentScrollHeight =
          currentScrollTop || PRELOADER.getCookie("currentScrollTop");
    ...
    if (
      CONFIG_BGIMAGE.preloaderEnable &&
      CONFIG_BGIMAGE.preloaderEnable != null
    ) {
      // 不设置预加载
      // 10s以后
    ...
    $(window)
      .scroll(function () {
        ...
        if ($(postPageId).length) {
          ...
          scrollPercent =
            ((scrollTop - headerH) /
              (postContentH + postTitleH + postMetaH + 80 - windowHeight)) *
            100;
        } else {
        ...
        $("#progress-bar").attr(
          "style",
          "width:" + scrollPercent + "%; display: block;"
        );
  ...
  const Annie_Archive = function () {
    ...
    $("#layout-archive-year h3").each(function () {
      $(
        "#layout-archive-year ul li[year = '" + $(this).attr("class") + "'"
      ).wrapAll("<div year = '" + $(this).attr("class") + "'></div>");
      ...
      $.ajax({
        ...
        success: function (data) {
          ...
          $(paginationContainer).append(
            result.fadeIn(delayTime).addClass("animation-zoom")
          );
